...
void ThongKe(int dataThongKe){
	...
	if (dataThongKe==1) 
	{
		if (n == 0)
		{
			...
			textcolor(14);
			gotoxy(xbieudo+15, ybieudo+5); printf("BIEU DO PHAN LOAI SINH VIEN KHA GIOI XUAT SAC");
			...
			// vẽ cột biểu đồ cuối
			xbieudo+=10; vebieudo=(100*demXS+100*demG+100*demK)/(dem*2);
			gotoxy(xbieudo, ybieudo+2); printf("%s", something);
			for(int i=ybieudo-vebieudo-1; i<=ybieudo; i++) 
			{
				textcolor(78);
	...
	if(dataThongKe==2)
	{
		if (n == 0)
		{
			...
			gotoxy(xbieudo+15, ybieudo+5); printf("BIEU DO PHAN TRAM SINH VIEN CUA CAC LOP");
